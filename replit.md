...
- Follow-up polish (same session): removed 4 redundant `TechnicalLabel` captions per user request — hero "SUPPORT CENTER" eyebrow, FAQ subtitle "INSTANT ANSWERS TO YOUR THORX QUERIES", chat header "THORX SUPPORT — LIVE" + its pulsing-dot indicator, and Contact tab's "DIRECT TEAM CONTACT" eyebrow. Chat header changed from `justify-between` (label left / barcode right) to `justify-end` (barcode only) since its left-side content was removed entirely. Verified with a fresh `tsc --noEmit` + production build + screenshots on all three tabs before/after.
...
## Setup notes (2026-08-05) — Phase 3 User Portal: Work section, Engine B (CPA Tasks) redesign

- Redesigned Engine B's full CPA Tasks Dashboard (`renderWorkSection()` in `client/src/pages/UserPortal.tsx`) to match the premium ivory/black/orange editorial system, replacing every remaining raw `.wireframe-border` element in this sub-section: header strip, the Active Task Modal (all three phases — timer/verify/done), the empty state, loading skeletons, task list cards, and the below-C-Rank locked-state panel. Reused exact patterns already shipped elsewhere (`DashboardCards` `CardShell`, Engine A's card hover treatment) rather than inventing new tokens: `rounded-2xl`, `border-2 border-black/15` with `hover:-translate-y-1 hover:border-black hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]` on task cards, `border-2 border-black` + hard shadow on the active-task panel (an intentionally stronger highlight state), pill-style PKR/PS reward chips, and `TechnicalLabel` for all micro-labels.
- No backend/gating logic touched — `engineBClickMutation`, `engineBVerifyMutation`, the phase state machine, and the `DEV_UNLOCK_RANK_GATES` rank-lock behavior are all unchanged (per explicit user instruction not to touch the lock while this pass is in progress); only the locked panel's visual styling was refreshed.
- Verified via `tsc --noEmit` (clean), no persistent browser console errors, and a temporary QA account + 3 throwaway `engine_b_tasks` rows (deleted afterward) confirmed `GET /api/engine-b/tasks` still returns data in the exact shape the redesigned cards consume.
- Noted but unrelated to this change: the dev workflow's shared `pg` pool (`server/db.ts`, `max: 20`, reused by both app queries and the `connect-pg-simple` session store) can end up queuing/timing out ("timeout exceeded when trying to connect") after a long-running dev session; a plain workflow restart clears it. Investigate if it recurs frequently.
...
## Setup notes (2026-08-22) — Beta v0.9 completion sprint
- Applied migrations 0010 (beta trust) + 0011 (survey_records + uniq_survey_network_tx) to the dev DB; deleted the temporary migration-runner test. All 17 test files / 242 tests pass; tsc clean.
- Built the beta frontend: BetaTrustLayer (mandatory honesty-rules gate z-gate token + floating feedback dock), SurveyWallPanel wired as Engine B content, conditional beta invite field on /auth register (driven by public GET /api/beta/status), Team Portal "Beta Control" section (feedback triage inbox + invite minting/kill switch) with nav entries on desktop (AdminSidebar) and mobile (AdminLayout).
- Owner credentials reset + live-login verified: founder thorx11dev@gmail.com / Thorx@Founder2026, tester thorx1111dev@gmail.com / Thorx@Tester2026.
- Purged 74 leftover automated-test CPA tasks from engine_b_tasks (0 remain); records cascade-deleted safely.
- Removed ALL CPA-task evidence from client/src (user portal Engine B is now Survey-Wall-only, Task Manager admin section + component deleted from both nav lists, FAQ/knowledgebase/landing/Terms/rank-unlock copy rewritten to surveys). Backend ledger pipeline intentionally untouched for history integrity. Guild weekly tasks (Engine C) untouched — unrelated feature.
- Seeded BETA_INVITE_REQUIRED into SYSTEM_CONFIG_DEFAULTS (was missing — Settings UI could not toggle the invite gate); removed dead CPA_NETWORKS seed key; updated stale CPA descriptions on ENGINE_B_* keys.
- Shipped PWA: manifest.webmanifest, production-only service worker (cache-first hashed assets, network-first navigations, zero interception of /api + non-GET), branded icons via scripts/generate-pwa-icons.mjs (sharp), iOS/Android meta tags in index.html, SW registration gated to import.meta.env.PROD so dev HMR stays intact. All assets verified serving 200.
- Added BETA_LAUNCH_RUNBOOK.md — network activation, invite gate, economy tuning, ops loop.

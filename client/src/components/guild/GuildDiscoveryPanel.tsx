...
export function GuildDiscoveryPanel() {
  ...
  const applyBlockedLabel = (guild: GuildDiscovery, inGuild: boolean, rankBlocked: boolean): { icon: React.ReactNode; text: string; title: string } => {
              ...
              >
                ...
                <div className="p-3 flex flex-col flex-1">
                  ...
                  <div className="flex justify-end mt-1.5">
                    <ChevronDown size={14} className={cn("text-black/30 transition-transform duration-300", isExpandedCard && "rotate-180")} />
                  </div>
                </div>
                ...
                {isExpandedCard && (
                  <div className="px-3 pb-3 pt-2 bg-black/[0.02] border-t-2 border-black/10 flex gap-2">
                    ...
                    <button
                      ...
                      title={applied ? "Applied" : applyDisabled ? applyBlockedLabel(guild, inGuild, rankBlocked).title : "Apply to join"}
                    ...
                    >
                      {applied ? <><ShieldCheck className="size-3" /> Applied</> : applyDisabled ? <>{applyBlockedLabel(guild, inGuild, rankBlocked).icon} {applyBlockedLabel(guild, inGuild, rankBlocked).text}</> : <>Apply <ArrowRight className="size-3" /></>}
                    </button>
                  </div>
                )}
              </motion.article>
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-3 md:px-4 pb-4 pt-1 bg-black/[0.02] border-t-2 border-black/10 flex gap-2">
                    ...
                    >
                      {applied ? <><ShieldCheck className="size-3.5" /> Applied</> : applyDisabled ? <>{applyBlockedLabel(guild, inGuild, rankBlocked).icon} {applyBlockedLabel(guild, inGuild, rankBlocked).text}</> : <>Apply <ArrowRight className="size-3.5" /></>}
                    </button>
                  </div>
                )}
              </motion.div>

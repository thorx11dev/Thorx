...
function MetaRow({
  ...
  return (
    <div className="flex items-start gap-3">
      <div className="w-7 h-7 rounded-lg border-[1.5px] border-[#111]/10 bg-zinc-50 flex items-center justify-center shrink-0 mt-0.5">
        <Icon size={13} className="text-zinc-400" />
      </div>
      <div className="min-w-0">
        <div className="text-[9px] font-black uppercase tracking-widest text-zinc-400 mb-0.5">{label}</div>
        <div className={cn("text-xs font-bold text-[#111] break-all", mono && "font-mono")}>{value}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <Sheet open={!!log} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="right" className="w-full sm:max-w-lg p-0 flex flex-col gap-0 overflow-hidden">
        {/* Header */}
        <SheetHeader className="p-6 pb-4 border-b-[1.5px] border-[#111]/10 text-left space-y-2 shrink-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {log.actorRole && (
              <span className="inline-flex px-2 py-1 rounded-full border-[1.5px] border-[#111]/20 bg-white text-[9px] font-black uppercase tracking-widest text-zinc-500">
                {formatRole(log.actorRole)}
              </span>
            )}
          </div>
          <SheetTitle className="font-black text-base uppercase tracking-tight text-[#111] leading-snug">
            {log.description}
          </SheetTitle>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="border-t-[1.5px] border-[#111]/5" />
          ...
          <section className="space-y-2">
            ...
            <div className="rounded-xl border-[1.5px] border-[#111]/10 bg-zinc-50 p-3 space-y-1">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[9px] font-black uppercase tracking-widest text-zinc-400">Type</span>
                <span className="text-[10px] font-black uppercase tracking-widest text-[#111]">{log.targetType || "—"}</span>
              </div>
              ...
              <section className="space-y-3">
                ...
                <div className="rounded-xl border-[1.5px] border-[#111] overflow-hidden">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y-[1px] divide-[#111]/5">
                      {Object.entries(diff).map(([field, { before, after }]) => (
                        <tr key={field} className="hover:bg-zinc-50/50">
                          <td className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-[#111] whitespace-nowrap">
                            {humanizeKey(field)}
                          </td>
                          <td className="px-3 py-2 text-[10px] font-mono text-zinc-500 break-all max-w-[110px]">
                            {renderDetailValue(before)}
                          </td>
                          <td className="px-3 py-2 text-[10px] font-mono font-bold text-[#111] break-all max-w-[110px]">
          ...
          {hasExtras && (
            <>
              ...
              <section className="space-y-3">
                ...
                <div className="rounded-xl border-[1.5px] border-[#111]/10 bg-zinc-50 divide-y-[1px] divide-[#111]/5 overflow-hidden">
                  {Object.entries(extras).map(([key, value]) => (
                    <div key={key} className="px-3 py-2 flex items-start justify-between gap-3">
                      ...
                      <span className="text-[10px] font-bold text-[#111] break-all text-right">
